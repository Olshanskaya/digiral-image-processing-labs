...
if __name__ == '__main__':
    img = cv2.imread("Bildschirmfoto.2016.04.06.um_.00.30.50.jpg")
    # cv2.imshow("orig", img)
    ...
    cv2.imwrite('linear_image.jpg', linear_image)
    ...
    cv2.imwrite('equalize.jpg', equ_image)
    ...
    cv2.imwrite('equalize.jpg', vis)
    fig2 = plt.figure()
    ax = fig2.add_subplot()
    steps = [x for x in range(0, 256)]
    print(steps)
    # print(histg_linear.list())
    ax.bar(steps, histg_linear.transpose().tolist()[0], 1, color="black")
    ax.bar(steps, histg.transpose().tolist()[0], 1, color ="blue")
    ax.bar(steps, histg_equalize.transpose().tolist()[0], 1, color ="green")
    # plt.hist(gray_img, edgecolor='blue', linewidth=1.2)
    # plt.hist(equ_image, edgecolor='yellow', linewidth=2)
    # plt.hist(linear_image, edgecolor='black', linewidth=2)
